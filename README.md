...
```mermaid
flowchart LR
    A[Insumos: metodología,<br/>código, métricas] --> B[Preparación segura:<br/>lectura, anonimización]
    B --> C[Agente IA<br/>ReAct]
    C --> D[Evaluación de hallazgo:<br/>severidad e impacto]
    D --> E[Reporte<br/>preliminar]
    E --> F[Revisión<br/>humana]
    F -->|aceptado| G[(Memoria SQLite:<br/>hallazgos validados)]
    G -.alimenta futuras validaciones.-> C
    H[(Elasticsearch:<br/>normativa SBS)] -.evidencia normativa.-> C
```
...
```mermaid
flowchart TD
    A[Pregunta del validador] --> B[Agente decide<br/>usar RAG]
    B --> C[HyDE genera<br/>documento hipotético]
    C --> D[Búsqueda híbrida:<br/>BM25 + kNN]
    D --> E[RRF fusiona<br/>los rankings]
    E --> F{¿Hay evidencia<br/>suficiente?}
    F -->|Sí| G[Respuesta con cita:<br/>documento, página,<br/>resolución, artículo]
    F -->|No| H[Fallback explícito:<br/>no se inventa normativa]
```
